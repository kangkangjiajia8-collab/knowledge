...
sources:
- id: compiler-input
  resource: source://ga4/tables/events_.md
  title: 'Compiler input: tables/events_.md'
  last_modified: '2026-07-20T03:27:52Z'
  sha256: c0b8237ec9cc3253248ea5965af4bcd6d42d94bdea187647f1861a199414eb1f
semantic:
  ...
  relations:
  - type: part_of
    target: ga4_obfuscated_sample_ecommerce
    evidence: from the ga4_obfuscated_sample_ecommerce dataset
  - type: is_a
    target: sharded BigQuery table
    evidence: is a sharded BigQuery table
  - type: measures
    target: Event Count
    evidence: 'Metrics: Event Count'
  - type: measures
    target: User Count
    evidence: 'Metrics: User Count'
  - type: measures
    target: Day Count
    evidence: 'Metrics: Day Count'
  - type: measures
    target: New User Count
    evidence: 'Metrics: New User Count'
  - type: measures
    target: Average Transactions Per Purchaser
    evidence: 'Metrics: Average Transactions Per Purchaser'
  - type: measures
    target: Average Pageviews
    evidence: 'Metrics: Average Pageviews'
  - type: measures
    target: Average Spend Per Purchase Session By User
    evidence: 'Metrics: Average Spend Per Purchase Session By User'
  - type: measures
    target: Overall Average Spend Per Purchase Session
    evidence: 'Metrics: Overall Average Spend Per Purchase Session'
  - type: part_of
    target: events_
    evidence: The events_YYYYMMDD table, created daily
  - type: part_of
    target: events_
    evidence: events_intraday_YYYYMMDD (for streaming export)
  - type: applies_to
    target: batch_event_index
    evidence: batch_event_index (INTEGER)
  - type: applies_to
    target: batch_ordering_id
    evidence: batch_ordering_id (INTEGER)
  - type: applies_to
    target: batch_page_id
    evidence: batch_page_id (INTEGER)
  - type: applies_to
    target: event_date
    evidence: event_date (STRING)
  - type: applies_to
    target: event_timestamp
    evidence: event_timestamp (INTEGER)
  - type: applies_to
    target: event_previous_timestamp
    evidence: event_previous_timestamp (INTEGER)
  - type: applies_to
    target: event_name
    evidence: event_name (STRING)
  - type: applies_to
    target: event_value_in_usd
    evidence: event_value_in_usd (FLOAT)
  confidence: 0.95
